...
class Grammar(object):
    ...
    def __init__(self, file_name):
        ...
        self.start_rule, self.codon_size = None, parameter.Codon_size

        self.min_path, self.max_arity, self.min_ramp = None, None, 2
    ...
    def read_bnf_file(self, file_name):
        ...
        with open(file_name, 'r') as bnf:
            ...
            for rule in finditer(self.ruleregex, content, DOTALL):
                ...
                # Finding the structured NT
                current_rulename = rule.group('rulename')
                inner = current_rulename[1:-1] if current_rulename.startswith('<') and current_rulename.endswith('>') else current_rulename
                tokens = [t.strip() for t in inner.split(',')]
                if len(tokens) >= 2 and 'structured' in tokens[1:]:
                    if current_rulename not in self.struct_nts:
                        self.struct_nts.append(current_rulename)
                ...
                for p in finditer(self.productionregex,
                    ...
                    GE_BETWEEN_regex = r'GE_BETWEEN:\((?P<start>\d+)-(?P<end>\d+)\)'
                    m_between = match(GE_BETWEEN_regex, p.group('production'))
                    if m_between:
                        try:
                            start_range = int(m_between.group('start'))
                            end_range = int(m_between.group('end'))
