...
def sort_non_dominated(population):
    ...
    OPTIMIZED VERSION: Uses vectorized NumPy operations for better performance.

    :param population: A list of individuals to select from.

    :returns: A ParetoInfo object with Pareto fronts information.
    ...
    n = len(population)
    maximise = parameter.Fitness_function.maximise

    # OPTIMIZATION: Extract all fitness values to NumPy matrix
    fitness_matrix = np.array([ind.fitness for ind in population])

    # OPTIMIZATION: Vectorized computation of domination matrix
    # dominates_matrix[i, j] = True means i dominates j
    for obj_idx, is_max in enumerate(maximise):
        f = fitness_matrix[:, obj_idx]

        if is_max:
            better = f[:, np.newaxis] > f[np.newaxis, :]
            as_good = f[:, np.newaxis] >= f[np.newaxis, :]
        else:
            better = f[:, np.newaxis] < f[np.newaxis, :]
            as_good = f[:, np.newaxis] <= f[np.newaxis, :]
        ...
        if obj_idx == 0:
            ...
            all_as_good = all_as_good & as_good
            any_better = any_better | better

    dominates_matrix = all_as_good & any_better

    # OPTIMIZATION: Compute domination counts using matrix operations
    domination_count_array = np.sum(dominates_matrix.T, axis=1)

    # Fill pareto data structures
    for i, p in enumerate(population):
        pareto.domination_count[p] = int(domination_count_array[i])

        # Find all individuals dominated by p
        dominated_indices = np.where(dominates_matrix[i, :])[0]
        pareto.dominated_solutions[p] = [population[j] for j in dominated_indices]

        # *p* belongs to the first front if domination count is 0
        if pareto.domination_count[p] == 0:
    ...
    # Compute the subsequent fronts (keep original logic for consistency)
    while len(pareto.fronts[i]) > 0:
        ...
        for p in pareto.fronts[i]:
            ...
            # Decrement the domination counter of each solution dominated by *p*
            for q in pareto.dominated_solutions[p]:
                pareto.update_domination_count(q, False)

                # If the counter reaches 0, the solution is added to the next front
...
def dominates(individual1, individual2):
    ...
    # Get numbers of fitness functions.
    maximise = parameter.Fitness_function.maximise
    n_ffs = parameter.Fitness_function.num_obj

    # Check how many fitness values are equal.
    equal_fit = [False] * n_ffs

    # Iterate over all fitness values and fitness functions.
    for v1, v2, is_max, i in zip(individual1.fitness, individual2.fitness, maximise, range(n_ffs)):
        if v1 == v2:
            equal_fit[i] = True

        elif not compare_fitnesses(v1, v2, is_max):
            return False
...
def compare_fitnesses(ind1_value, ind2_value, ff_maximise):
    """
...
def get_population_iqr(population, n_objectives):
    ...
    for m in range(n_objectives):
        ...
        ind.fitness[m], reverse=parameter.Fitness_function.maximise[m])

        # Get the inter-quartile fitness ranges for the current objective.
        q1 = sorted_pop[ceil(len(sorted_pop) * 75 / 100) - 1].fitness[m]
        q3 = sorted_pop[ceil(len(sorted_pop) * 25 / 100) - 1].fitness[m]
    ...
    for front in pareto.fronts:
        ...
        if len(front) > 0:
            ...
            for m in range(pareto.n_objectives):
                ...
                front = sorted(front, key=lambda ind:
                ind.fitness[m], reverse=parameter.Fitness_function.maximise[m])
                ...
                for index in range(1, solutions_num - 1):
                    ...
                    pareto.crowding_distance[front[index]] += \
                        ((front[index + 1].fitness[m]) - (front[index - 1].fitness[m])) / \
...
class ParetoInfo:
    ...
    def __init__(self):
        ...
        try:
            self.n_objectives = parameter.Fitness_function.num_obj
